        ...
        try:
            with transaction.atomic():
                ...
                user = User.objects.create_user_with_role_name(**validated_data,
                                                               is_active=False,
                                                               role_name=variable_system.JOB_SEEKER)
...
class CompanyRegisterSerializer(serializers.ModelSerializer):
    ...
    since = serializers.DateField(required=False,
                                  input_formats=[variable_system.DATE_TIME_FORMAT["ISO8601"],
                                                 variable_system.DATE_TIME_FORMAT["Ymd"]],
                                  allow_null=True)
        ...
        try:
            with transaction.atomic():
                ...
                user = User.objects.create_user_with_role_name(**validated_data,
                                                               is_active=False,
                                                               has_company=True,
                                                               role_name=variable_system.EMPLOYER)
...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_avatar_url(self, user):
        ...
        return variable_system.AVATAR_DEFAULT["AVATAR"]

    def get_job_seeker_profile(self, user):
        if user.role_name == variable_system.JOB_SEEKER:
    ...
    def get_company(self, user):
        if user.role_name == variable_system.EMPLOYER:
            ...
            company_logo_url = company_logo.get_full_url() if company_logo else variable_system.AVATAR_DEFAULT["COMPANY_LOGO"]
