...
class JobSeekerJobPostActivityViewSet(viewsets.ViewSet,
    ...
    def job_seeker_job_posts_activity_chat(self, request):
        ...
        queryset = user.jobpostactivity_set \
            .select_related(
                "job_post__company__user",
                "job_post__company__logo"
            ) \
            .order_by('-create_at', '-update_at')
            
        page = self.paginate_queryset(queryset)
        
        def build_result(activity):
            company = activity.job_post.company
            return {
                "id": activity.id,
                "userId": company.user.id,
                "fullName": company.user.full_name,
                "userEmail": company.user.email,
                "companyId": company.id,
                "companyName": company.company_name,
                "companySlug": company.slug,
                "companyImageUrl": company.logo.get_full_url() if company.logo else None,
                "jobPostTitle": activity.job_post.job_name
            }
        
        data = [build_result(activity) for activity in (page or queryset)]
